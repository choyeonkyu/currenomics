...
# FRED API에서 한국 고용율 ID
SERIES_ID = 'LREMTTTTKRM156S' 
...
def extract(**kwargs):
    ...
    employment_data = fred.get_series(SERIES_ID, observation_start='2019-01-01')

    # pandas로 포팅 시 테이블 스키마 명시
    # employment_data.index : 첫행 (년월일)
    # employment_data.values : 값
    employment_df = pd.DataFrame({
        "date": employment_data.index,
        "employment_rate": employment_data.values
    })

    # S3로 CSV저장 (tmp 경로에 저장)
    upload_csv_to_S3(employment_df, True, ds_nodash)
